...
int read_txtfft(char *file_name, float **buf, int *len) {
  ...
  while (1) {
    char lbuf[256];
    char *rv = fgets(lbuf, 255, input_file);
    if (!rv) {
    ...
    int ord_num;
    float val;
    int rc = sscanf(lbuf, "%d %f", &ord_num, &val);
    if (rc != 2) {
      fprintf(stderr, "hibas sor: %s\n", lbuf);
      if (fclose(input_file)) {
        perror("error while reading input file (fclose3)");
        return -1;
      }
      return -1;
    }
    if (buf_size == read_len) {
      ...
      if (!tmp) {
        ...
        if (fclose(input_file)) {
          perror("error while reading input file (fclose4)");
          return -1;
        }
        return -1;
      }
      memcpy(tmp, data, buf_size*sizeof(float));
      buf_size *= 2;
      free(data);
      data = tmp;
    }
